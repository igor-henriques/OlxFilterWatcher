    ...
    public async Task<IActionResult> OnPostAsync(string returnUrl = null, CancellationToken cancellationToken = default)
    {
        ...
        if (ModelState.IsValid)
        {
            ...
            if (result.Succeeded)
            {
                ...
                await _emailSender.SendEmailAsync($"Please confirm your account by <a href='{HtmlEncoder.Default.Encode(callbackUrl)}'>clicking here</a>.", 
                    new string[] { Input.Email }, "Confirm your email",
                    cancellationToken);
